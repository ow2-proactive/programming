    ...
    public void submit(String command) {
        ...
        try {
            ...
            switch (OperatingSystem.getOperatingSystem()) {
                case unix:
                    pb = new ProcessBuilder(CentralPAPropertyRepository.PA_GCMD_UNIX_SHELL.getValue(), "-c", command);
                    break;
                case windows:
    ...
    public void submit(List<List<String>> commandList) {
        ...
        for (int i = 0; i < commandList.size(); i++) {
            ...
            try {
                ...
                pb.redirectErrorStream(true);
                Process p = pb.start();
                InputStreamMonitor<List<String>> streamMonitor = new InputStreamMonitor<List<String>>(p.getInputStream(),
                                                                                                      command,
                                                                                                      "Process" + i,
                                                                                                      logger);
                streamMonitor.start();
