...
public class BrokerAddressMap {
    ...
    public static BrokerAddressMap createFromMappingFile(AMQPConnectionParameters defaultParams,
        ...
        for (Object key : properties.keySet()) {
            ...
            if (property.startsWith(BROKER_KEY_PREFIX)) {
                ...
                if (brokerKey == null) {
                    throw new IllegalArgumentException("Broker's host:port is empty for property: " + property);
                }
                ...
                String password = properties.getProperty(brokerName + ".password", AMQPConstants.DEFAULT_PASSWORD);
                String vhost = properties.getProperty(brokerName + ".vhost", AMQPConstants.DEFAULT_VHOST);
                try {
                    mapping.put(brokerKey,
                                new AMQPConnectionParameters(host, Integer.valueOf(port), username, password, vhost));
    ...
    public BrokerAddressMap(AMQPConnectionParameters defaultParams, Map<String, AMQPConnectionParameters> mapping) {
