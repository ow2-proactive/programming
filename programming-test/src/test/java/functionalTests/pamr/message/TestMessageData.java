...
public class TestMessageData extends MessageFunctionalTest {
    ...
    public void testDataRequest() {
        ...
        try {
            ...
            Assert.fail("There is a problem in the " + MessageType.DATA_REQUEST + " implementation:" + " the message " +
                        msgGen.getMessage() + " cannot be reconstructed from its " + "raw byte form, because:" +
                        e.getMessage());
        }
    }
    ...
    public void testDataReply() {
        ...
        try {
            ...
            Assert.fail("There is a problem in the " + MessageType.DATA_REPLY + " implementation:" + " the message " +
                        msgGen.getMessage() + " cannot be reconstructed from its " + "raw byte form, because:" +
                        e.getMessage());
        }
    }
    ...
    private class DataMessageGenerator extends MessageGenerator {
        ...
        protected void testInvalidMessage() {
            ...
            try {
                ...
                Assert.fail("Problem with " + type + " implementation: Attemp to reconstruct the message " + m +
                            " with a corrupted " + DataMessage.Field.SRC_AGENT_ID +
            ...
            try {
                ...
                Assert.fail("Problem with " + type + " implementation: Attemp to reconstruct the message " + m +
                            " with a corrupted " + DataMessage.Field.DST_AGENT_ID +
                            " from its raw byte array representation actually succeeded!");
            } catch (MalformedMessageException e) {
                // success
            }
        ...
        private DataMessage corruptAgentId(long invalidAgentId, int fieldOffset) throws MalformedMessageException {
            byte[] corruptedMsg = msg.toByteArray();
            TypeHelper.longToByteArray(invalidAgentId, corruptedMsg, Message.Field.getTotalOffset() + fieldOffset);
