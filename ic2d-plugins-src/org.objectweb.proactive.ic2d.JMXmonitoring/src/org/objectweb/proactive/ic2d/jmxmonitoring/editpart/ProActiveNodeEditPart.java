...
public final class ProActiveNodeEditPart extends AbstractMonitoringEditPart {
    private ProActiveNodeObject castedModel;
    ...
    public void update(Observable o, Object arg) {
        ...
        switch (notif.getMVCNotification()) {
            ...
            case REMOVE_CHILD:
                ...
                // In order to keep model<->ep synced during the search of the concerned edit part
                // it's preferable to keep the retrieval of the edit part 
                Display.getDefault().asyncExec(new Runnable() {
                    public final void run() {
                        // Get the associated edit part (find it from children edit parts)					
                        for (final Object object : getChildren()) {
                ...
                break;
            // If ADD or REMOVE CHILDREN notification simply sync children edit parts by refreshing children !
            case ADD_CHILDREN:
    ...
    @Override
    public ProActiveNodeObject getCastedModel() {
        if (castedModel == null) {
            castedModel = (ProActiveNodeObject) getModel();
        }
        return castedModel;
    }
    ...
    protected IFigure createFigure() {
        ProActiveNodeObject model = getCastedModel();
