...
public class CommunicationFigure extends PolylineConnection {
    ...
    protected void outlineShape(Graphics g) {
        ...
        if (WorldEditPart.displayTopology() || showMe) {
            ...
            super.outlineShape(g);
            // TODO: fix method drawShape and use it instead
            // super.outlineShape(g)
    ...
    // if (xSource == xTarget) {
    // if (ySource > yTarget) {
    // this.setBounds(new Rectangle(xTarget - shapeY, yTarget, shapeY * 2,
    // Math.abs(ySource -
    // yTarget)));
    // g.drawArc(xTarget - shapeY, yTarget, shapeY * 2, Math.abs(ySource -
    // yTarget), 90, -180);
    //
    // //g.drawRectangle(this.getBounds());
    // } else {
    // this.setBounds(new Rectangle(xSource - shapeY, ySource, shapeY * 2,
    // Math.abs(ySource -
    // yTarget)));
    // g.drawArc(xSource - shapeY, ySource, shapeY * 2, Math.abs(ySource -
    // yTarget), 90, 180);
    //
    // }
    ...
    // g.drawArc(xTarget, yTarget - (shapeX / 4), Math.abs(xSource - xTarget),
    // shapeX / 2, 0,
    // 180);
    // //g.drawRectangle(this.getBounds());
    // } else {
    ...
    // Math.abs(xSource - xTarget), shapeX / 2));
    // g.drawArc(xSource, ySource - (shapeX / 4), Math.abs(xSource - xTarget),
    // shapeX / 2, 0,
    // -180);
    // // g.drawRectangle(this.getBounds());
    // }
