...
public class BasicChartContainerObject {
    ...
    /**
     * This method allows to automatically find potential active objects that
     * are timer aware and create a chart from the gathered data.
     * 
     * @param object
     *            an AbstractObject instance that can be ActiveObject,
     *            NodeObject, RuntimeObject, HostObject or WorldObject
     */
    public final void recognizeAndCreateChart(final AbstractData<?, ?> object) {
        ...
        } else if (object instanceof ProActiveNodeObject) {
            createFromNodeObject((ProActiveNodeObject) object);
    ...
    protected final void createFromWorldObject(final WorldObject object) {
        ...
        // them
        List<HostObject> children = object.getMonitoredChildrenAsList();
        for (HostObject o : children) {
            createFromHostObject(o);
        }
    }
    ...
    protected final void createFromHostObject(final HostObject object) {
        // Get All nodes from the Virtual Machine object and create charts from
        // them
        List<RuntimeObject> children = object.getMonitoredChildrenAsList();
        for (RuntimeObject o : children) {
            createFromVMObject(o);
        }
    }
    ...
    protected final void createFromVMObject(final RuntimeObject object) {
        // Get All nodes from the Virtual Machine object and create charts from
        // them
        List<ProActiveNodeObject> children = object.getMonitoredChildrenAsList();
        for (ProActiveNodeObject o : children) {
            createFromNodeObject(o);
        }
    }

    protected final void createFromNodeObject(final ProActiveNodeObject object) {
        // Get All active objects from the node and create charts from them
        List<ActiveObject> children = object.getMonitoredChildrenAsList();
        for (ActiveObject o : children) {
            createFromAOObject(o);
        }
    }
    ...
    /**
     * Since each children can be identified, this method returns a specific
     * child
     * 
     * @param id
     *            The id of the child
     ...
     * 
     * @param forceRefresh
     *            If true each children model will be refreshed
     */
