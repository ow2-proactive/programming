...
public class Symmetric3Test {
	...
	public static void main(String[] args) {
		...
		try {
			...
			Key key2 = new Key(0, Zone.MAX_Y-10);
			Key key3 = new Key(Zone.MAX_X-10, 0);
			...
			peers.get(0).add(key2, (Serializable) msg);
			peers.get(0).add(key3, (Serializable) msg);
			
			long startLookup = System.currentTimeMillis();
			
			List<Object> results = new LinkedList<Object>();
			for (int i=0; i<xMessages/3; i++) {
				results.add(peers.get((int) 0).lookup(key1));
				results.add(peers.get((int) 0).lookup(key2));
				results.add(peers.get((int) 0).lookup(key3));
			}
