...
public class PAArgumentComponentLoader extends ArgumentComponentLoader {
	...
	protected void resolveComponentContainer(final Set<String> loaded,
		...
		logger.debug("[PAArgumentComponentLoader]   RCC "+ container.toString() + " TL:" + topLevelDefinition.toString() );
		...
		for (final Component comp : container.getComponents()) {
			...
			if (definition != null) {
				...
				if (defs.size() != 1 || !isShared(defs.get(0))) {
					...
					Node merged;
					logger.debug("[PAArgumentComponentLoader]   Merging F: "+ comp.toString() + " with supernode "+ d.toString() );
					try {
					...
					if (merged != comp) {
						...
						logger.debug("[PAArgumentComponentLoader]   Merging F: Container "+ container.toString() + " ... merged "+ merged.toString());
		...
		for (final Component comp : getNFComponents(container) ) {
			...
			if (definition != null) {
				...
				if (defs.size() != 1 || !isShared(defs.get(0))) {
					...
					Node merged;
					logger.debug("[PAArgumentComponentLoader]   Merging NF: "+ comp.toString() + " with supernode "+ d.toString() );
					try {
						merged = nodeMergerItf.merge(comp, d, nameAttributes);
					} catch (final MergeException e) {
						throw new CompilerError(ComponentErrors.MERGE_ERROR,
					...
					if (merged != comp) {
						// the container AST is a pure tree (i.e. no sharing), so simply
						// replace the merged node in the container.
						logger.debug("[PAArgumentComponentLoader]   Merging NF: Container "+ container.toString() + " ... merged "+ merged.toString());
	...
	protected ComponentDefinition resolveDefinitionExtension(
		...
		if (def.getExtends() != null) {
			...
			try {
				...
				logger.debug("[PAArgumentComponentLoader]   Merging(rde): "+ def.toString() + " ... Super: "+ def2.toString());
	...
	protected Definition resolveDefinitions(final Set<String> loaded,
		...
		for (int i = 1; i < nameList.size(); ++i) {
			...
			try {
				logger.debug("[PAArgumentComponentLoader]   Merging(rd): Node: "+ e.toString() + " ... Super: "+ d.toString());
